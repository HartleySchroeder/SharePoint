    ...
    var progressTimer,
      ...
      dialog = $( "#dialog" ).dialog({
        ...
        open: function() {
			...
			setTimeout(function() { checkWorkflowStatus(0) }, 1000);
...
function checkWorkflowStatus(itemID) {
	$().SPServices({
		operation: 'GetWorkflowDataForItem', 
		item: 'http://solveraade04/sites/DevSite/Lists/test_list/' + workflowArray[itemID].toString() + '_.000',
		completefunc: function(data,status){
			...
			if (activeWF == '2') {
				setTimeout(function() { checkWorkflowStatus(itemID) }, 500);
			}
			else if (activeWF == '5') {
				itemID = itemID + 1;
